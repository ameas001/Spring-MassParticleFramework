...
{
	const int NUM_PARTICLES = 100;
	const int SQRT = (int) sqrt(NUM_PARTICLES);
	particles = std::vector<Particle*>(NUM_PARTICLES);
	for (int i = 0; i < NUM_PARTICLES; ++i)
	{
		// Initialize all particles at the provided location
		...
		double time = 3.0;
		double size = 2.0;
		Color4 col = Color4(0.0, 0.0, 0.0, 1.0);
	...
	for(m = 0; m < SQRT; m++)
	{
		if(m + 1 < SQRT)
		{
		  for(int o = 0; o < SQRT; o++)
		    springConnections.push_back(SpringJoint(particles[SQRT * m + o], particles[SQRT * (m + 1) + o], 110.0, 0.1, 18.0));
		}
	  for(n = 0; n + 1 < SQRT; n++)
	    springConnections.push_back(SpringJoint(particles[SQRT * m + n], particles[SQRT * m + n + 1], 110.0, 0.1, 18.0));
	}
}
